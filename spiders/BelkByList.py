...
import re
class BelkByListSpider(scrapy.Spider):
    name = 'belkbylist'
    allowed_domains = ['belk.com']
    start_urls = ['https://www.belk.com/search/?prefn1=couponCodes&prefv1=SUPERSAVE&srule=Best-Matches&start=0&sz=60']
    def parse(self, response):
        try:
            ...
            yield from response.follow_all(css='a.page-next::attr(href)', callback=self.parse)
    ...
    def parse_detail(self, response):
        try:
            ...
            for sku_upc in utag_data_load["sku_upc"]:#upc all
                  ...
                  if int(inventory)>=3 and (coupon_price.__contains__('-') or float(price)/float(original_price)<=0.5 or float(coupon_price)/float(original_price)<=0.5):
